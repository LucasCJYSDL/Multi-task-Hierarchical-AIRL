...
from envir.visualize_dst import plot_hier_policy
import numpy as np
...
def reward_validate(agent, policy: Union[OptionPolicy, Policy, MHAOptionPolicy], training_episode, time_token, n_sample=-8, do_print=True):
    ...
    policy.train()
    # if (training_episode+1) % 50 == 0 or training_episode == 0:
    plot_hier_policy(trajs, training_episode, time_token, unique_token='eval')
    trajs_train = agent.collect(policy.state_dict(), n_sample, fixed=False)
    plot_hier_policy(trajs_train, training_episode, time_token, unique_token='train')
