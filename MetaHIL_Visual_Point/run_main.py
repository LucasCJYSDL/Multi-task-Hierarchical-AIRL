...
from vae_train import pretrain
...
def sample_batch(il: Union[MHAOptionAIRL], agent, n_sample, demo_sa_array):
...
def train_d(il: Union[MHAOptionAIRL], sample_sxar, demo_sxar, training_itr, n_step=10):
...
def learn(config: Config, msg="default"):
    ...
    il.store_exp_data(demo_sa_array)
    ...
    info_dict, cs_sample = reward_validate(sampling_agent, il.policy, time_token=time_token, training_episode=0, do_print=True)
