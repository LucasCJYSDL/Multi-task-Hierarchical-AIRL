...
@click.option(
    ...
    default='./d4rl_env/kitchen_demos_multitask/microwave_top_switch_hinge/')
@click.option(
    '--skip',
    '-s',
    type=int,
...
def pkl_to_torch(data_dir):
    env = gym.make('KitchenMetaEnv-v0')
    folder_list = []
    ...
    for i in range(len(folder_list)):
        ...
        for ind, file in enumerate(glob.glob(demo_dir + "*.pkl")):
            ...
            if ind < 10:
                train_trajs.append((torch.tensor(obs), torch.tensor(act), torch.tensor(rwd)))
            elif ind < 15:
                test_trajs.append((torch.tensor(obs), torch.tensor(act), torch.tensor(rwd)))
            else:
                print("Enough!")
                break
    ...
    torch.save(train_demos, 'KitchenMetaEnv-v0_sample_train.torch')
    torch.save(train_demos, 'KitchenMetaEnv-v0_sample_test.torch')
