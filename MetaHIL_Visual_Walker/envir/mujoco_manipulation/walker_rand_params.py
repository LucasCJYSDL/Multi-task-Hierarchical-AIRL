...
class WalkerRandParamsEnv(MujocoEnv, utils.EzPickle):
    ...
    def get_skill_idx(self):
        if self.sub_goal_vel_idx == 0 or self.sub_goal_vel_idx == 2:
            return 0 # speed up
        else:
            return 1 # slow down
    ...
    def _get_obs(self):
        ...
        if self.is_expert:
            obs_ext = np.concatenate([np.array([self.cur_vel]), obs, self.true_params])
        else:
            obs_ext = np.concatenate([np.array([self.cur_vel]), obs, self.context])
        return obs_ext
