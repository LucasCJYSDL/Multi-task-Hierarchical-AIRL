...
def no_option_loop(env, policy, is_expert, fixed, task_list=None, contain_context=False):
    env_num = env.env_num
    with torch.no_grad():
...
class VecSampler(object):
    ...
    def collect(self, policy_param, n_sample, fixed=False):
        ...
        if counter > 0:
            while counter > 0:
                trajs, trans_num = self.loop_func(self.vec_env, self.policy, self.is_expert, fixed=fixed,
            ...
            while counter < 0: # only used for testing, so don't need repeated sampling
                trajs, _ = self.loop_func(self.vec_env, self.policy, self.is_expert, fixed=fixed,
