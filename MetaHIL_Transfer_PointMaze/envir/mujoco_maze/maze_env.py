...
class MazeEnv(gym.Env):
    def __init__(
        ...
        self.sub_goal_num = 4
        # self.sub_goal_list = [np.array([21. / 3., 0.0]), np.array([21. * 2. / 3., 0.0]),
        #                       np.array([21., 0.0]), np.array([21., 14.])]
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        outer_reward, done = self._task.reward(next_obs)  # rwd from the outer task
