...
class SMILEStateOnlyDiscriminator(torch.nn.Module):
    def __init__(self, config: Config, dim_s=2, dim_a=2): # in SMILE, f(S, A)->f(S, S')
        super(SMILEStateOnlyDiscriminator, self).__init__()
        self.dim_a = dim_a
        self.dim_s = dim_s
        self.device = torch.device(config.device)
        n_hidden_d = config.hidden_discriminator
        ...
        self.discriminator = make_module(self.dim_s + self.dim_s, 1, n_hidden_d, activation) # TODO: only contain one context within the input
    ...
    def gradient_penalty(self, s, a, lam=10.):
        ...
        last_s = temp_s[-1]
        next_s = torch.cat([next_s, last_s.unsqueeze(0)], dim=0)
        ss = torch.cat([s, next_s], dim=-1).requires_grad_()

        d = self.discriminator(ss)

        gradients = torch.autograd.grad(outputs=d, inputs=ss, grad_outputs=torch.ones_like(d),
                                        create_graph=True, retain_graph=True, only_inputs=True)[0]
...
class StateOnlyDiscriminator(torch.nn.Module):
    def __init__(self, config: Config, dim_s=2, dim_a=2):
        super(StateOnlyDiscriminator, self).__init__()
        self.dim_a = dim_a
        self.dim_s = dim_s
        ...
        self.g_fn = make_module(self.dim_s, 1, n_hidden_d, activation)
        self.h_fn = make_module(self.dim_s, 1, n_hidden_d, activation)

        self.to(self.device)

    def get_unnormed_d(self, s, a): # keeping 'a' is only to keep the interface constant
        g = self.g_fn(s) # (N, 1)
        h = self.h_fn(s) # (N, 1)
        n_h = self.h_fn(s[1:]) # (N-1, 1)
        n_h = torch.cat([n_h, torch.zeros((1, 1), device=self.device, dtype=torch.float32)], dim=0) # (N, 1)

        return g + self.gamma * n_h - h
