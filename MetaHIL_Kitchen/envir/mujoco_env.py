...
def get_demo(train_path, test_path, n_traj, task_specific=False):
    ...
    for task_idx in train_set:
        # train_demos[task_idx] = {'context': train_set[task_idx]['context'], 'demos': train_set[task_idx]['demos']}
...
def run_model(config, n_task=1000, demo_per_task=10, env_name=None, expert_path=None):
    from model.option_policy import Policy, OptionPolicy
    ...
    for task_idx in range(n_task):
        ...
        with torch.no_grad():
            ...
            s_array = []
            r_array = []
            s, done = env.reset(context, is_expert=False), False
            ...
            while not done:
                ...
                ct = policy.sample_option(st, ct, fixed=False).detach()
                at = policy.sample_action(st, ct, fixed=False).detach()
                s_array.append(st)
                c_array.append(ct)
                a_array.append(at)
