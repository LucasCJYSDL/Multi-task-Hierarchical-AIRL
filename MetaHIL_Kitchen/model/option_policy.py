...
from utils.model_util import make_module, make_module_list, make_activation, init_weights_zeros, init_weights_normal
...
class MAMLPolicy(torch.nn.Module):
    def __init__(self, config: Config, dim_s=2, dim_a=2):
        ...
        self.dim_a = dim_a
        self.dim_s = dim_s
        self.dim_bt = config.dim_bt # the dimension of the bias transformation
        self.device = torch.device(config.device)

        # we use the same network structure as the one in the implementation of HIL
        n_hidden_pi = config.MAML_policy_hidden_dim
        # bias transformation trick
        self.bt_input = torch.nn.Parameter(torch.zeros(self.dim_bt, dtype=torch.float32, requires_grad=True, device=self.device))

        self.fc1 = nn.Linear(self.dim_s + self.dim_bt, n_hidden_pi)
        init_weights_normal(self.fc1.weight, std=0.01)
        init_weights_zeros(self.fc1.bias)

        self.fc2 = nn.Linear(n_hidden_pi, n_hidden_pi)
        init_weights_normal(self.fc2.weight, std=0.01)
        init_weights_zeros(self.fc2.bias)
        # two-head structure
        self.pre_head = nn.Linear(n_hidden_pi, self.dim_a)
        init_weights_normal(self.pre_head.weight, std=0.01)
        init_weights_zeros(self.pre_head.bias)

        self.post_head = nn.Linear(n_hidden_pi, self.dim_a)
        init_weights_normal(self.post_head.weight, std=0.01)
        init_weights_zeros(self.post_head.bias)
    ...
    def forward(self, state_input, post_update):
        # build bias transformation
        batch_num = state_input.shape[0]
        bt_fc = torch.zeros(batch_num, self.dim_bt, device=self.device, dtype=torch.float32)  # build container for bias transformation
        bt_fc += self.bt_input

        fc_input = torch.cat([bt_fc, state_input], dim=1)
        ...
        if not post_update:
            fc_output = self.pre_head(fc_output)
        else:
            fc_output = self.post_head(fc_output)

        # fc_output = self.tanh(fc_output) # since it does not use gaussian sampling, we need extra constraints here to limit its range

        return fc_output

    def functional_forward(self, state_input, weights, post_update):
        # build bias transformation
        batch_num = state_input.shape[0]
        bt_fc = torch.zeros(batch_num, self.dim_bt, device=self.device,
                            dtype=torch.float32)  # build container for bias transformation
        bt_fc += weights['bt_input']

        fc_input = torch.cat([bt_fc, state_input], dim=1)

        hid_out = F.linear(fc_input, weight=weights['fc1.weight'], bias=weights['fc1.bias'])
        hid_out = F.relu(hid_out)
        hid_out = F.linear(hid_out, weight=weights['fc2.weight'], bias=weights['fc2.bias'])
        hid_out = F.relu(hid_out)

        if not post_update:
            final_out = F.linear(hid_out, weight=weights['pre_head.weight'], bias=weights['pre_head.bias'])
        else:
            final_out = F.linear(hid_out, weight=weights['post_head.weight'], bias=weights['post_head.bias'])
