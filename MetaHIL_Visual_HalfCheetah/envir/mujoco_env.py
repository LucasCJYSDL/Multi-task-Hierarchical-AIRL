...
class MujocoEnv(object):
    ...
    def get_cur_vel(self):
        return self.env.get_cur_vel()
...
def collect_demo(config, n_task=1000, demo_per_task=10, data_type='train',
    ...
    dim_s, dim_a = env.state_action_size(is_expert=True)
    ...
    demo_set = {}
    init_context_list = [np.array([-3.0]), np.array([-2.5]), np.array([-2.0]), np.array([-1.5]), np.array([-1.0]),
                         np.array([-0.5]), np.array([-0.0]), np.array([0.5]), np.array([1.0]), np.array([1.5]),
                         np.array([2.0]), np.array([2.5]), np.array([3.0])]
    for task_idx in range(n_task):
        if task_idx < len(init_context_list):
        ...
        while len(trajs) < demo_per_task:
            with torch.no_grad():
                ...
                ext_s = np.zeros((len(s)+1, ), dtype=np.float32)
                ext_s[0] = env.get_cur_vel()
                ext_s[1:] = s
                s = ext_s
                # print("222: ", s)
                ...
                while not done:
                    ...
                    c_array.append(ct)
                    if not is_manual:
                        at = policy.sample_action(st[:, 1:], fixed=True)  # eliminate the randomness of the expert policy
                    else:
...
if __name__ == '__main__':
    ...
    # collect_demo(config, n_task=13, demo_per_task=10, data_type='test', expert_path='./exp_model/HalfCheetahVel/6549.torch')
    #
    get_demo_stat('HalfCheetahVel-v0_sample_train.torch')
    get_demo_stat('HalfCheetahVel-v0_sample_test.torch')
