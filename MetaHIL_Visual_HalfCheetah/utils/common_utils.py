...
# from envir.visualize_dst import draw_vel_change_hier_policy
import numpy as np
...
def reward_validate(agent, policy: Union[OptionPolicy, Policy, MHAOptionPolicy], training_episode, time_token, n_sample=-8, do_print=True):
    policy.eval() # TODO: using eval()
    trajs = agent.collect(policy.state_dict(), n_sample, fixed=True) # for evaluation only
    policy.train()
    # draw_vel_change_hier_policy(trajs, training_episode, unique_token='eval', time_token=time_token)
    # trajs_train = agent.collect(policy.state_dict(), n_sample, fixed=False)
    # draw_vel_change_hier_policy(trajs_train, training_episode, unique_token='train', time_token=time_token)
...
def get_dirs(seed, exp_type="gail", env_type="mujoco", env_name="HalfCheetah-v2", msg="default"):
    ...
    return log_dir, save_dir, train_set_name, test_set_name, pretrain_name, time_token
