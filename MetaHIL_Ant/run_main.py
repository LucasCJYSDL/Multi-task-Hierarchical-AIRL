...
def make_il(config: Config, dim_s, dim_a, dim_cnt, cnt_limit):
    ...
    if config.use_hppo:
        ppo = HierarchicalPPO(config, il.policy)
    else:
...
def learn(config: Config, msg="default"):
    ...
    sampling_agent = Sampler(seed, env, il.policy, is_expert=False, repeat_num=config.context_repeat_num,
                             task_list=test_contexts)
