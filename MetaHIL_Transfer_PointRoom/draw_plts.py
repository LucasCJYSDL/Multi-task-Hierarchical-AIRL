...
def plot():
    ...
    file_dir = {'HPPO-init': [16971, 16755, 14696, 14762], 'HPPO': [13276, 13355, 13422, 13492],
                'DAC': [13573, 13639, 13707, 13779], 'PPO': [14650, 14691, 14718, 14744]}
    ...
    for alg, dir_name_list in file_dir.items():
        ...
        for dir_name in dir_name_list:
            ...
            for i in range(temp_len):
                if temp_step[i] > step_limit:
                    break
                if temp_value[i] > 1250:
                    temp_value[i] = 1250
    ...
    # pal = sns.xkcd_palette((['red', 'blue', 'green', 'yellow']))
    g = sns.relplot(x="Step", y="Reward", hue='algorithm', kind="line", ci="sd", data=data_frame, legend='brief')

    leg = g._legend
    leg.set_bbox_to_anchor([0.69, 0.65])  # coordinates of lower left of bounding box [0.75, 0.35], [0.55, 0.75], [0.4, 0.7]
...
class MovAvg(object):
    ...
    def __init__(self, window_size=30, mode='avg'):
