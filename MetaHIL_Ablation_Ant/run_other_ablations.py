...
def learn(config: Config, msg="default"):
    ...
    if config.n_thread == 1:
        sampling_agent = Sampler(seed, env, il.policy, is_expert=False, repeat_num=config.context_repeat_num,
                                 task_list=test_contexts, contain_context=True)
    else:
        sampling_agent = VecSampler(seed, env_name, config.n_thread, il.policy, is_expert=False,
                                    repeat_num=config.context_repeat_num, task_list=test_contexts, contain_context=True)
